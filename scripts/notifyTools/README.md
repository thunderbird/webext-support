...
This NotifyTools API also provides a `onNotifyBackground` event which can be registered in the background page. The `notifyTools.js` script provides a `notifyBackground()` function, which will send a nitification to that listener and will await its return value.
