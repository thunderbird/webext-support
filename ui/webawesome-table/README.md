...
├─ build.js               # Build script that creates the add-on from the source,
│                         # executed via "npm run build".
│
├─ node_modules/          # Installed npm dependencies (created by "npm ci").
│                         # Source of the vendored Web Awesome files copied
...
│  ├─ dialog.js           # Renders the table data and handles sorting and
│  │                      # filtering.
│  ├─ spinner.gif         # Loading indicator used by the dialog.
│  └─ vendor/             # Vendored library files (generated during build,
│     └─ webawesome/      # not committed). Web Awesome files copied during
│                         # "npm run build".
│
└─ dist/                  # Generated output (not committed).
   ├─ extension.xpi       # Packaged extension generated from src/
   └─ source.zip          # Source archive required for ATN submissions. Used by
                          # reviewers to inspect the source and reproduce the
                          # add-on (contains all non-generated files)
```
